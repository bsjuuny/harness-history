...
2. **Harness pull + 규칙**
   ...
   - 긴 세션 재개, 70% 이상 context, cache read 폭증, provider fallback 이슈가 있으면
     `skills/context-session-budget/SKILL.md` 도 읽는다.
...
3. **누적 맥락**
   ...
   - 세션이 길어졌으면 전체 과거 대화를 이어 붙이지 말고, 현재 작업·완료분·변경 파일·결정·제약·테스트·남은 일만 압축한다.
